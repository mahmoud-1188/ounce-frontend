...
const STORE_REGISTRY = [
  ...
  {
    key: RFID_KEY, name: "rfidCfg", def: RFID_DEFAULTS,
    clean: (v) => ({ ...RFID_DEFAULTS, ...(v || {}) }),
  },
  {
  ...
  // الأسئلة المحفوظة (QueryBuilderPage.jsx) — تفضيلٌ شخصي محلي بحت،
  // نفس منزلة menuOrder/navLayout، لا بيانات عملٍ تحتاج مصدر حقيقةٍ خادميًّا.
  { key: SAVED_QUERIES_KEY, name: "savedQueries", def: [], clean: (v) => (Array.isArray(v) ? v : []) },
  {
