...
async function fetchGoldPriceSAR() {
  ...
  const perGramSar = Number(res.world24 ?? res.perGram);
  if (!Number.isFinite(perGramSar) || perGramSar <= 0) throw new Error("invalid_price");
  // ⚠ perGram هو السعر العالمي؛ سياسة الإدارة (الزيادة) تصل معه من الخادم
  const markup = res.markup && Number(res.markup.value) > 0 ? { mode: res.markup.mode === "percent" ? "percent" : "amount", value: Number(res.markup.value) } : null;
  return { perGram: perGramSar, asOf: res.asOf || "", markup, source: res.source || "auto", policyAt: res.policyAt || null, policyBy: res.policyBy || null,
    price24: Number(res.price24) > 0 ? Number(res.price24) : applyHqMarkup(perGramSar, markup) };
}

// ═══ زيادة الإدارة على السعر العالمي ═══
//
// ⚠ السعر الذي تعمل به الفروع = **السعر العالمي + زيادةٌ تعتمدها الإدارة** (ريالًا
//   للجرام أو نسبةً). العالمي يتحرّك كل دقيقتين (جلبٌ آلي) أو يُدخل يدويًّا؛
//   والزيادة ثابتةٌ حتى تغيّرها الإدارة — فتُطبَّق آليًّا على كل سعرٍ جديد.
const applyHqMarkup = (world24, markup) => {
  const w = Number(world24) || 0;
  if (!markup || !(Number(markup.value) > 0)) return Math.round(w * 100) / 100;
  const v = Number(markup.value) || 0;
  const out = markup.mode === "percent" ? w * (1 + v / 100) : w + v;
  return Math.round(out * 100) / 100;
};

const markupLabel = (markup) => (!markup || !(Number(markup.value) > 0)) ? "" : (markup.mode === "percent" ? `${markup.value}٪` : `${fmtMoney(markup.value)}/جم`);
